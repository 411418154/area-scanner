...
class AreaScanner3DWidget(QWidget):
    ...
    def update_from_frame(self, frame, buffer_frame_count: int = 1) -> None:
        ...
        targets = self._filter_targets_by_roi(frame_info["targets"])
        self._append_target_history(targets)
    ...
    def _append_target_history(self, targets: Sequence[dict]) -> None:
        ...
        stale = [tid for tid in self._target_trails if tid not in live_ids]
        for tid in stale:
