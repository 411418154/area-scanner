...
# True = 從 02 01 04 03 06 05 08 07 開始記錄
# False = Data Port 收到什麼就立刻記錄什麼
...
# 2. 基本工具
# ==========================================================
def bytes_to_hex_stream(data: bytes) -> str:
    """每個 byte 中間只加空格，不做排版。"""
    if not data:
        return ''
...
def open_log_file() -> tuple[Path, object]:
    ...
    log_path = LOG_DIR / f'tlv_stream_raw_{ts}.txt'
    f = open(log_path, 'w', encoding='utf-8')
    return log_path, f


# ==========================================================
# 3. 主記錄流程：不分封包，直接一路串下去
# ==========================================================
def log_raw_stream(data_serial: serial.Serial, log_file) -> None:
    ...
    while True:
        ...
        if not started:
            ...
            text = bytes_to_hex_stream(prebuffer)
            if text:
                log_file.write(text)
                chunk_count += 1
            log_file.flush()
        ...
        text = bytes_to_hex_stream(incoming)
        if text:
            log_file.write(text)
            chunk_count += 1
